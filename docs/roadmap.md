...
| **OPS-02** | Ejecutar `seed_dedicatorias.php` en **prod** (Plesk → Scheduled Tasks → «Run a PHP script», **seleccionar PHP 8.4 explícitamente**) — pendiente desde el 2026-07-23 | 15 min | ⏳ 2026-08-27: PHP 8.4 ya seleccionado en la tarea, corre limpio. `--dry-run` confirma trabajo real pendiente (`grupos={1383} canónicas nuevas=157 alias nuevos=266`, sobre 1.228 dedicatorias / 1.242 alias ya existentes — probablemente por las ~803 marchas aceptadas en OPS-01) — falta relanzar sin `--dry-run` para que escriba |
